...
public class PaymentMethodController {
    ...
    public ResponseEntity<List<PaymentMethodResponse>> getPaymentMethods() {
        List<PaymentMethodResponse> paymentMethods = paymentMethodService.getPaymentMethods();
    ...
    PaymentMethodResponse result = paymentMethodService.createPaymentMethod(paymentMethodRequest);
    return ResponseEntity.created(URI.create("/payment_methods/" + result.getId())).body(result);
    }
    ...
    @PutMapping("/{id}")
     public ResponseEntity<Object> updatePaymentMethod(@PathVariable Long id,@RequestBody PaymentMethodRequest request) {
        ...
        try {
            ...
            return ResponseEntity.ok("El método de pago fue actualizado.");
        } 
        catch (NoSuchPaymentMethodException nspe){
